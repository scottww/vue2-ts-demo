  ...
  <div class="expandable-select" :class="{ disabled: isDisabled }">
    <div class="select-wrapper">
      ...
      <div
        class="select-option"
        :class="{
          selected: selectedOptions.length === 0,  // 全选选项的选中状态
      ...
      <div
        ...
        :class="{
          ...
          disabled: isOptionDisabled(option.value) || (isSingleSelect && selectedOptions.length === 1 && !selectedOptions.includes(option.value))
        }"
        @click="isOptionDisabled(option.value) ? null : (isSingleSelect && selectedOptions.length === 1 && !selectedOptions.includes(option.value) ? null : toggleSelection(option.value))"
        :style="{
...
export default {
  ...
  data() {
    return {
      selectedOptions: [...this.value], // 初始选中的选项
  ...
  methods: {
    toggleSelection(value) {
      if (value === 'all') {
        ...
        // 如果选项都取消了，恢复“全部”选项  tips: 一个非'全部'选项都没有的情况，’全部‘选项无法被取消选中状态
        if (this.selectedOptions.length === 0) {
