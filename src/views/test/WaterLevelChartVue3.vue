...
// 提取X轴日期
const dates = waterLevelData.map(item => item.date);
// 提取实时水位数据
const realTimeData = waterLevelData.map(item => item.realTime);
// 提取警戒水位数据（常量）
const warningData = waterLevelData.map(() => 7.0);
// 提取保证水位数据（常量）
const guaranteeData = waterLevelData.map(() => 10.0);
