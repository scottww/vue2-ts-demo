...
import cloudyImg from "@/assets/bigScreen/oneMap/sunny.png";
import partlyCloudyImg from "@/assets/bigScreen/oneMap/rain3.png";
...
export default {
  ...
  methods: {
    ...
    getChartOption() {
      ...
      const days = displayData.map((d) => d.day);
      const highTemps = displayData.map((d) => d.highTemp);
      const lowTemps = displayData.map((d) => d.lowTemp);

      // 天气图标映射
      const iconMap = {
        sunny: this.sunnyImg,
        cloudy: this.sunnyImg,
        "partly-cloudy": this.rain3Img,
      ...
      return {
        ...
        series: [
          ...
          },
          {
            name: "天气",
            type: "scatter",
            data: displayData.map(() => 0), // 必须长度一致
            symbol: function (params) {
              // 防止 dataIndex 越界
              if (!displayData[params.dataIndex]) return "circle";
              const iconKey = displayData[params.dataIndex].icon;
              return "image://" + iconMap[iconKey];
            },
            symbolSize: 30,
            tooltip: { show: false }
          }
