  ...
  data() {
    return {
      ...
      defaultWeatherData: [
        ...
        { day: '周三', highTemp: 35, lowTemp: 25, icon: 'cloudy' },
        { day: '周四', highTemp: 34, lowTemp: 24, icon: 'cloudy' },
        { day: '周五', highTemp: 34, lowTemp: 24, icon: 'cloudy' },
        { day: '周六', highTemp: 33, lowTemp: 23, icon: 'cloudy' },
        { day: '周日', highTemp: 31, lowTemp: 21, icon: 'cloudy' }
      ]
    };
  },
  computed: {
    // 优先使用自定义数据，没有则使用默认数据
    displayData() {
      return this.weatherData.length > 0 ? this.weatherData : this.defaultWeatherData;
    }
  },
  mounted() {
    this.initChart();
