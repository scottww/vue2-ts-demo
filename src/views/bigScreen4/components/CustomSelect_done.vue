<template>
  <div class="custom-select" ref="selectRef" :style="{ width: typeof width === 'number' ? width + 'px' : width }">
    <!-- 选择框 -->
    <div class="selected__value" @click="toggleDropdown">
      <div class="diamond">
        <span class="diamond__label" :title="selectedLabel" :class="{ 'is-placeholder': !selectedLabel }">
          {{ selectedLabel || placeholder }}
        </span>
        <span class="diamond__arrow" :class="{ open: isOpen }">▼</span>
        <span v-if="clearable && selectedValue !== undefined && selectedValue !== ''" class="diamond__clear-btn"
      ...
      <transition name="fade">
        <div v-if="isOpen" class="dropdown-menu" :style="dropdownStyle" ref="dropdownMenu">
          ...
          <ul :key="selectedValue">
            <li v-for="option in filteredOptions" :key="option.id" :class="{ active: option.value == selectedValue }"
              :title="selectedLabel" @click.stop="selectOption(option)">
              {{ option.label }}
            </li>
