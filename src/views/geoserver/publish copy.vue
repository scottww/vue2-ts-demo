...
export default {
  ...
  methods: {
    ...
    async loadStyles() {
      ...
      // 1) 先拉全局样式列表（JSON）
      try {
        const resGlobal = await axios.get(`${this.geoserverUrl}/rest/styles.json`, {
          auth: this.auth
        });
      ...
      try {
        const resWs = await axios.get(`${this.geoserverUrl}/rest/workspaces/${this.workspace}/styles.json`, {
          auth: this.auth
        });
      ...
      const workers = new Array(concurrency).fill(0).map(async () => {
        while (idx < names.length) {
          ...
          try {
            ...
            let sldText = null;
            try {
              const r = await axios.get(`${this.geoserverUrl}/rest/workspaces/${this.workspace}/styles/${name}`, {
                auth: this.auth,
              ...
              // fallback to global
              try {
                const r2 = await axios.get(`${this.geoserverUrl}/rest/styles/${name}`, {
                  auth: this.auth,
                  headers: { Accept: "application/vnd.ogc.sld+xml" },
                  responseType: "text"
                });
      ...
      this.styleList = Array.from(styles.values())
