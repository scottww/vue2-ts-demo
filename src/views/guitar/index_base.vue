...
export default {
  ...
  computed: {
    ...
    fretPositions() {
      const list = [];
      for (let i = 1; i <= this.frets; i++) list.push(i);
      return list;
    },
    renderStrings() {
      return this.invertOrder ? [...this.strings].reverse() : this.strings;
    }
  },
  methods: {
    noteAt0(stringNote, fret) {
    ...
    noteAt(stringNote, fret) {
      const idx = this.noteToIndex[stringNote];
      const noteIdx = fret > 0 ? (idx + fret) % 12 : idx;
      return this.useFlats
        ? this.noteNamesFlat[noteIdx]
        : this.noteNamesSharp[noteIdx];
    },
    ...
    isDotMarker(fret, stringIdx) {
      ...
      if (fret === 12) return stringIdx === 1 || stringIdx === 3; // double dots middle
      return stringIdx === 2;
    }
  }
};
