...
export default {
  ...
  computed: {
    ...
    fretWidths() {
      ...
      return Array.from(
        { length: this.frets },
        (_, i) => maxWidth - ((maxWidth - minWidth) * i) / (this.frets - 1)
      );
    },
    fretOffsets() {
      const offsets = [];
      let acc = 0;
      for (let w of this.fretWidths) {
        offsets.push(acc);
        acc += w;
      }
      return offsets; // 第n品左边距
    }
  },
  methods: {
    ...
    fretStyle(fret) {
      const width = this.fretWidths[fret - 1];
      return {
        width: width + "px",
        minWidth: width + "px",
        position: "absolute", // 改为绝对定位
        left: this.fretOffsets[fret - 1] + 40 + "px" // +40 是琴枕宽
      };
    },
    dividerStyle(fret, index) {
      const left = (fret === 0 ? 0 : this.fretOffsets[fret - 1]) + 40; // +琴枕宽
      return {
        position: "absolute",
        top: 0,
        bottom: 0,
        left: left + "px",
        width: index === 0 ? "4px" : "2px",
        background: "#999",
        opacity: 0.7
      };
    },
