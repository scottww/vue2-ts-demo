...
export default {
  ...
  data() {
    return {
      map: null,
      zoom: 12,
      rippleLayers: [], // 存告警点和其圈
      maxRadius: 30,    // 扩散圈最大半径
      numRipples: 3     // 同时显示圈数量
    };
  },
              ...
              styles.push(
                new Style({
                  image: new CircleStyle({
                    ...
                    stroke: new Stroke({ color: "rgba(255,51,51,0.5)", width: 2 })
                  })
                })
              );
            });
      ...
      setInterval(() => {
        this.rippleLayers.forEach(r => {
          for (let i = 0; i < r.radii.length; i++) {
            r.radii[i] += 1.5; // 每帧增大半径
            if (r.radii[i] > this.maxRadius) {
              r.radii[i] = 0; // 超过最大值重置
            }
          }
        });
