    ...
    <a-card class="converter-card" title="坐标转换工具">
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }" layout="horizontal">
        ...
        <a-form-item label="输入坐标">
          <textarea v-model="inputText" rows="6" class="textarea" placeholder='例如 [[664373,3307642],[664374,3307639]]'></textarea>
        </a-form-item>
      ...
      <div v-if="result.length" class="result-section">
        <h4>转换结果（JSON）</h4>
        <textarea :value="resultText" rows="6" class="textarea" readonly></textarea>
      </div>
    </a-card>
  </div>
</template>
