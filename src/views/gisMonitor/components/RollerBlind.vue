<template>
  <div class="roller-blind-container" :style="containerStyle">
    ...
    <!-- 右边滑块区域 -->
    <div
      class="roller-blind-slider-container"
...
<script>
// 导入默认背景图片
import defaultBackgroundImage from "@/assets/images/YearPicker/卷帘背景.png";
...
export default {
  ...
  methods: {
    ...
    drag(e) {
      ...
      this.sliderPosition = Math.max(
        0,
        Math.min(100, this.startPosition + deltaPosition)
      );
  ...
  watch: {
    initialPosition(newVal) {
      this.sliderPosition = newVal;
    },
  },
};
</script>
