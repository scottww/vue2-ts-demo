...
export class AnimatedRouteLayer {
  ...
  createArrowStyle() {
    const { width, arrowSize, arrowInterval } = this.options;
    ...
    // 2️⃣ 光流层（整条曲线，透明度变化模拟光流）
    const alpha = 0.3 + 0.7 * Math.abs(Math.sin(this.time * Math.PI * 2));
    styles.push(
      new Style({
        geometry: new LineString(this.coordinates),
        stroke: new Stroke({
          color: `rgba(255,255,255,${alpha})`, // 白色光流，可以改成蓝白渐变模拟
          width
        })
      })
    );
