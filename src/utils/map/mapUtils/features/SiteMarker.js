...
export class SiteMarker {
  ...
  createMarkerElement() {
    ...
    icon.style.borderColor = this.borderColor; // 设置图标边框颜色
    icon.innerHTML = `<div class="site-marker-icon-inner" style="color: ${this.iconColor}">${this.icon}</div>`;
  ...
  // 更新站点信息
  updateInfo(name, status, statusColor, borderColor, icon, iconColor) {
    ...
    if (iconElement) {
      iconElement.style.borderColor = this.borderColor;
    }
    if (iconInner) {
